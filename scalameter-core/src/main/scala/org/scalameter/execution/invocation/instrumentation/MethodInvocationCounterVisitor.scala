...
private[instrumentation] class MethodInvocationCounterVisitor(
    cv: ClassVisitor,
    pattern: InvocationCountMatcher,
    counterClass: String,
  ...
  override def visitMethod(access: Int,
    ...
    if (className != counterClass &&
      ...
      val method =
        MethodSignature(className = className, methodName = name, Type.getArgumentTypes(desc).map(_.getClassName): _*)
      rawMethods += method
    }
