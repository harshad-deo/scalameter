...
trait Measurer[V] extends Serializable { self =>
  ...
  def measure[T](context: Context,
                 measurements: Int,
                 setup: T => Any,
                 tear: T => Any,
                 regen: () => T,
                 snippet: T => Any): Seq[Quantity[V]]
...
object Measurer {
  ...
  class Default extends Timer with IterationBasedValue {
    ...
    def measure[T](context: Context,
                   ...
                   tear: T => Any,
                   regen: () => T,
                   snippet: T => Any): Seq[Quantity[Double]] = {
      var iteration = 0
  ...
  trait OutlierElimination[V] extends Measurer[V] {
    ...
    abstract override def measure[T](context: Context,
                                     measurements: Int,
                                     setup: T => Any,
                                     tear: T => Any,
                                     regen: () => T,
  ...
  trait Noise extends Measurer[Double] {
    ...
    abstract override def measure[T](context: Context,
      ...
      val withnoise = observations map { x =>
        x.copy(value = x.value + noise(x.value))
      }
  ...
  class GarbageCollectionCycles extends Measurer[Int] {
    ...
    def measure[T](context: Context,
                   ...
                   tear: T => Any,
                   regen: () => T,
                   snippet: T => Any): Seq[Quantity[Int]] = {
      var iteration = 0
  ...
  object BoxingCount {
    ...
    /** Creates BoxingCount measurer that counts boxing of all primitive values -
      ...
      */
    def all() =
      new BoxingCount(classOf[Boolean],
                      classOf[Char],
                      classOf[Byte],
                      classOf[Short],
                      classOf[Int],
    ...
    /** Creates BoxingCount measurer that counts boxing of all primitive values -
      *  boolean, char, byte, short, int, long, float and double.
      */
    def allWithoutBoolean() =
      new BoxingCount(classOf[Char],
                      classOf[Byte],
                      classOf[Short],
                      classOf[Int],
                      classOf[Long],
