...
object MeasurementDeserializer extends StdDeserializer[Measurement[_]](classOf[Measurement[_]]) {
  def deserialize(p: JsonParser, ctx: DeserializationContext): Measurement[_] = {
    ...
    val measurement = for {
      ...
      params <- Option {
        ...
        ctx
          .findContextualValueDeserializer(paramsType, null)
          .deserialize(p, ctx)
...
/** Deserializer for maps with keys whose are subtype of [[org.scalameter.PicklerBasedKey]].
  ...
  */
class PicklerBasedMapDeserializer[MapKey <: PicklerBasedKey[_]](clazz: Class[immutable.Map[MapKey, Any]],
                                                                keyCreator: (String, Pickler[_]) => MapKey)
...
object PicklerBasedMapSerializerResolver extends Serializers.Base {
  private val allowedTypes: Map[Class[_], JsonSerializer[_]] = Map(
    classOf[Key[_]] -> new PicklerBasedMapSerializer[Key[_]](classOf[immutable.Map[Key[_], Any]])
      .asInstanceOf[JsonSerializer[_]],
    classOf[Parameter[_]] -> new PicklerBasedMapSerializer[Parameter[_]](classOf[immutable.Map[Parameter[_], Any]])
      .asInstanceOf[JsonSerializer[_]]
  )
...
object PicklerBasedMapDeserializerResolver extends Deserializers.Base {
  private val allowedTypes: Map[Class[_], JsonDeserializer[_]] = Map(
    classOf[Key[_]] -> new PicklerBasedMapDeserializer[Key[_]](
      ...
      (name, _) => Key.parseKey(name)).asInstanceOf[JsonDeserializer[_]],
    classOf[Parameter[_]] -> new PicklerBasedMapDeserializer[Parameter[_]](
      classOf[immutable.Map[Parameter[_], Any]],
      (name, pickler) => Parameter(name)(pickler)).asInstanceOf[JsonDeserializer[_]]
  )
