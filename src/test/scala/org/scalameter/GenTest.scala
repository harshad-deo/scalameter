...
class GenTest extends FunSuite {
  ...
  test("A generator should be renamed") {
    ...
    assert(gen.dataset.toList == (100 to 500 by 100).map(x => Parameters(Parameter[Int]("size") -> x)),
           gen.dataset.toList)

    val renamed = gen.rename("size" -> "thatSize")
    assert(renamed.dataset.toList == (100 to 500 by 100).map(x => Parameters(Parameter[Int]("thatSize") -> x)),
           renamed.dataset.toList)
