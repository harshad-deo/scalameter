...
trait PersistorTest { this: Matchers =>
  ...
  def compareHistory(actual: History[_], expected: History[_]): Unit = {
    ...
    actual.results.zip(expected.results).foreach {
      case (actualR, expectedR) =>
        ...
        actualR._3.measurements.zip(expectedR._3.measurements).foreach {
          case (actualM, expectedM) =>
