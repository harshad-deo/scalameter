...
trait InvocationCount extends Measurer[Map[String, Long]] {
  ...
  def measure[T](
    ...
    def measureSnippet(value: T): Any = {
      ...
      invocations += Quantity(
        methodTable
          .map(_.toString)
          .zip(MethodInvocationCounter.counts())
          .groupBy(_._1)
          .map {
            case (k, v) =>
  ...
  override def beforeExecution(context: Context) = {
    val jar = context.goe(exec.measurers.instrumentedJarPath,
                          sys.error("Measurer.beforeExecution should be called after Measurer.prepareContext"))
  ...
  override def afterExecution(context: Context) = {
    val jar = context.goe(exec.measurers.instrumentedJarPath,
                          sys.error("Measurer.afterExecution should be called after Measurer.prepareContext"))
    jar.delete()
  }
}
