...
object Statistics {
  ...
  /** Compares two alternative sets of measurements given a significance level `alpha`.
	 ...
	 */
  case class ConfidenceIntervalTest(strict: Boolean, alt1: Seq[Double], alt2: Seq[Double], alpha: Double)
      extends Standard2WayTest {
    val diffM = m1 - m2
    val diffS = sqrt(s1 * s1 / n1 + s2 * s2 / n2)
    val ndf =
      math.round(pow(pow(s1, 2) / n1 + pow(s2, 2) / n2, 2) / (pow(pow(s1, 2) / n1, 2) / (n1 - 1) + pow(pow(s2, 2) / n2,
                                                                                                       2) / (n2 - 1)))
  ...
  /** Computes the confidence interval of the two alternatives.
	 ...
	 */
  case class OverlapTest(alt1: Seq[Double], alt2: Seq[Double], alpha: Double, noiseMagnitude: Double)
      extends Standard2WayTest {
  ...
  def SSE(alternatives: Seq[Seq[Double]]): Double = {
    val means: Seq[Double] = for (a <- alternatives) yield mean(a)
    val doubleSumTerms = for ((alternative, mean) <- alternatives zip means; yij <- alternative)
      yield (yij - mean) * (yij - mean)
