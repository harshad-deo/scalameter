...
class InvocationCountMatcherTest extends FunSuite with Matchers {
  ...
  test("ClassMatcher.Descendants should match classes") {
    val matcher1 =
      ClassMatcher.Descendants(classOf[collection.immutable.LinearSeq[_]], direct = false, withSelf = false)
    ...
    val matcher2 =
      ClassMatcher.Descendants(classOf[collection.immutable.LinearSeq[_]], direct = true, withSelf = false)
    ...
    val matcher3 =
      ClassMatcher.Descendants(classOf[collection.immutable.LinearSeq[_]], direct = false, withSelf = true)
  ...
  test("MethodMatcher.Full should match methods") {
    ...
    matcher.matches("valueOf", Type.getMethodDescriptor(classOf[String].getMethod("valueOf", classOf[Int]))) should ===(
      true)
    matcher.matches("valueOf", Type.getMethodDescriptor(classOf[String].getMethod("valueOf", classOf[Long]))) should ===(
      false)
    matcher.matches("toString", Type.getMethodDescriptor(classOf[String].getMethod("valueOf", classOf[Long]))) should ===(
      false)
  }
}
